...
class TEST_CryApiThread(QThread):   # 실제 클래스랑 이름만 맞추면 됨
    db_ready = pyqtSignal(float)
    text_ready = pyqtSignal(str)

    _next_id = 0   # 클래스 전체에서 공유하는 카운터

    def __init__(self, *args, interval_sec=1, parent=None, **kwargs):
        super().__init__(parent)
        ...
        # --- 여기서 각 인스턴스마다 다른 값 세팅 ---
        self.my_id = TEST_CryApiThread._next_id
        TEST_CryApiThread._next_id += 1
